...
router.post('/orders', async (req, res) => {
    ...
    try {
        ...
        if (!orderTotal_id) {
            // Buscar un `OrderTotal` activo (ya sea por grupo o usuario)
            if (orderGroup_id) {
                ...
                console.log('Buscando OrderTotal para el usuario y mesa:', { user_id, table_id });
        ...
        // 2. Crear un nuevo `OrderTotal` si no existe
        if (!orderTotal_id) {
        ...
        // 3. Insertar productos en `OrderDetail`
        ...
        console.log('Actualizando total acumulado en OrderTotal:', totalAmount);
        await db.query(
